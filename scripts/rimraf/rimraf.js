...
const pathsToDelete = dirsNameWithinPackages.map(dir => folderNamesToRemove.map(folderNameToRemove => `${dir}/${folderNameToRemove}`)
).flat().map(modifiedPath => path.join(process.cwd(), 'packages', modifiedPath))

pathsToDelete.forEach(pathToDelete => {
  rimraf(pathToDelete, {}, (e) => console.log(e))
})